...
func (d *globalSecretsDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *globalSecretsDataSource) Read(ctx context.Context, _ datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for page := 1; ; page++ {
		endpoint := fmt.Sprintf("%s/api/v1/secrets?page=%d&perPage=50", d.client.Host, page)
		...
		if httpResp.StatusCode != http.StatusOK {
			resp.Diagnostics.AddError(
				...
				fmt.Sprintf("GET /secrets returned status %d", httpResp.StatusCode),
			)
			return
		}
